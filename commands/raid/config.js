    ...
    if (subcommand === "setrole") {
      ...
      try {
        ...
        const embed = new EmbedBuilder()
          ...
          .setDescription(`Raid commands will now only show users with the **${role.name}** role.`)
          .addFields(
        ...
        const errorEmbed = new EmbedBuilder()
          ...
          .setDescription("Failed to save the role configuration. Please try again.")
          .setColor(0xff0000);
      ...
      try {
        ...
        if (!guildConfig || !guildConfig.raidRoleId) {
          const embed = new EmbedBuilder()
            ...
            .setDescription("No raid role is currently configured. All users will be shown in raid commands.")
            .addFields(
              { name: "To configure", value: "Use `/raidconfig setrole` to set a raid role", inline: false }
            )
            .setColor(0x999999);
          ...
          const role = interaction.guild.roles.cache.get(guildConfig.raidRoleId);
          ...
          const embed = new EmbedBuilder()
            ...
            .addFields(
              { 
                name: "Raid Role", 
                value: role ? `<@&${guildConfig.raidRoleId}>` : `⚠️ Role deleted (${guildConfig.raidRoleName})`, 
                inline: true 
              },
              { name: "Configured by", value: `<@${guildConfig.configuredBy}>`, inline: true },
              { name: "Last updated", value: `<t:${Math.floor(guildConfig.updatedAt.getTime() / 1000)}:R>`, inline: true }
            )
            .setColor(role ? 0x0099ff : 0xffaa00)
            .setTimestamp();
            
          if (!role) {
            embed.setDescription("⚠️ The configured role no longer exists. Please update the configuration.");
          }
        ...
        const errorEmbed = new EmbedBuilder()
          .setTitle("❌ Configuration Error")
          .setDescription("Failed to fetch the role configuration. Please try again.")
          .setColor(0xff0000);
      ...
      try {
        ...
        if (!guildConfig || !guildConfig.raidRoleId) {
          ...
          const embed = new EmbedBuilder()
            ...
            .setDescription("Raid commands will now show all users in the server.")
            .addFields(
        ...
        const errorEmbed = new EmbedBuilder()
          .setTitle("❌ Configuration Error")
          .setDescription("Failed to clear the role configuration. Please try again.")
          .setColor(0xff0000);
